...
**Latest MyAIPs release: 0.105** · September 17, 2026 · [Release notes](#whats-new)
...
- Built-in AI Assistant for OpenAI-compatible chat endpoints: configure a provider and model, send current document/layer information plus an optional canvas preview, and let the model inspect and edit the active document through the bundled canvas tools
...
- Privacy: no telemetry or tracking. When you use the built-in AI Assistant, your prompt and current document/layer information go to the provider you configure; the canvas image is sent only when preview is enabled. Update checks contact GitHub only to check for a newer version. Settings live locally, and the installer doesn't change your file-extension preferences
