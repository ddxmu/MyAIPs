...
void ui_script_scripting_guide_opens_from_help() {
  ...
  dismisser->stop();
  CHECK(saw_guide);
  CHECK(guide_text.contains(QStringLiteral("MyAIPs Scripting Guide")));
...
void ui_ai_setup_dialog_opens_from_help() {
  ...
  CHECK(blurb.contains(QStringLiteral("named \"myaips\"")));
...
void ui_ai_setup_blurb_reports_missing_and_flatpak_forms() {
  ...
  CHECK(text.contains(QStringLiteral("Reuse an existing matching MyAIPs connection")));
...
void ui_ai_setup_examples_keep_installation_prompt_stable() {
  ...
  CHECK(all.contains(QStringLiteral("visible MyAIPs window")));
