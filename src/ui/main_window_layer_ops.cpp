...
void MainWindow::clear_system_clipboard() {
  if (auto* clipboard = QApplication::clipboard(); clipboard != nullptr) {
    ...
    clipboard->clear();
    patchy_system_clipboard_signature_ = clipboard_image_signature(image_from_clipboard(clipboard));
  }
}
...
void MainWindow::set_system_clipboard_image(const QImage& image) {
  if (auto* clipboard = QApplication::clipboard(); clipboard != nullptr) {
    ...
    if (png_buffer.open(QIODevice::WriteOnly) && image.save(&png_buffer, "PNG")) {
      // Keep an explicit lossless representation for macOS and other native
      // clipboards that may otherwise choose a scaled display rendition.
...
void MainWindow::clear_internal_clipboard_on_external_change() {
  const auto current_signature = clipboard_image_signature(image_from_clipboard(QApplication::clipboard()));
...
void MainWindow::copy_selection() {
  ...
  auto clipboard_image = qimage_from_pixel_buffer(clipboard_->pixels);
  set_clipboard_image_ppi(clipboard_image, document().print_settings().horizontal_ppi,
                          document().print_settings().vertical_ppi);
  set_system_clipboard_image(clipboard_image);
...
void MainWindow::copy_merged() {
  ...
  auto image = qimage_from_document(document(), true).copy(QRect(copy_rect.x, copy_rect.y, copy_rect.width, copy_rect.height));
  set_clipboard_image_ppi(image, document().print_settings().horizontal_ppi,
                          document().print_settings().vertical_ppi);
...
void MainWindow::paste_clipboard() {
  ...
  if (clipboard_.has_value() && !clipboard_->pixels.empty()) {
    ...
    const auto image = image_from_clipboard(QApplication::clipboard());
    if (image.isNull()) {
